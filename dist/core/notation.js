...
var _notation = _interopRequireDefault(require("./notation.error"));

var _notation2 = _interopRequireDefault(require("./notation.glob"));
...
function () {
  ...
  function Notation(source, options) {
    ...
    if (arguments.length === 0) {
      src = {};
    } else if (!_utils.default.isCollection(source)) {
      throw new _notation.default(ERR.SOURCE);
    }

    this.options = options;
    this._source = src;
    this._isArray = _utils.default.isArray(src);
  } // --------------------------------
  // INSTANCE PROPERTIES
  // --------------------------------
  ...
  _createClass(Notation, [{
    ...
    value: function eachValue(notation, callback) {
      ...
      Notation.eachNote(notation, function (levelNotation, note, index, list) {
        level = _utils.default.hasOwn(level, note) ? level[note] : undefined;
    ...
    value: function inspect(notation) {
      ...
      Notation.eachNote(notation, function (levelNotation, note) {
        var lastNoteNormalized = _utils.default.normalizeNote(note);

        if (_utils.default.hasOwn(level, lastNoteNormalized)) {
      ...
      result.parentIsArray = _utils.default.isArray(parent);
      return result;
    }
    /**
    ...
    value: function inspectRemove(notation) {
      ...
      var parentIsArray = _utils.default.isArray(parent);

      var lastNote = Notation.last(notation);

      var lastNoteNormalized = _utils.default.normalizeNote(lastNote);

      var result;

      if (_utils.default.hasOwn(parent, lastNoteNormalized)) {
        result = {
          notation: notation,
          has: true,
    ...
    value: function get(notation, defaultValue) {
      ...
      if (this.options.strict && arguments.length < 2 && !result.has) {
        ...
        throw new _notation.default(msg + "'".concat(notation, "'"));
      }
    ...
    value: function set(notation, value) {
      ...
      if (!notation.trim()) throw new _notation.default(ERR.NOTATION + "'".concat(notation, "'"));
      var level = this._source;
      var currentIsLast, nCurrentNote, nNextNote, nextIsArrayNote;
      Notation.eachNote(notation, function (levelNotation, note, index, list) {
        currentIsLast = index === list.length - 1;
        nCurrentNote = nNextNote || _utils.default.normalizeNote(note);
        nNextNote = currentIsLast ? null : _utils.default.normalizeNote(list[index + 1]);

        if (_utils.default.isArray(level) && typeof nCurrentNote !== 'number') {
          var parent = Notation.parent(levelNotation) || 'source';
          throw new _notation.default("Cannot set string key '".concat(note, "' on array ").concat(parent));
        } // check if the property is at this level


        if (_utils.default.hasOwn(level, nCurrentNote)) {
          ...
          if (currentIsLast) {
            ...
            if (overwrite) {
              ...
              throw new _notation.default('Cannot overwrite an existing value in strict mode.');
            }
          } else {
    ...
    value: function merge(notationsObject) {
      ...
      if (!_utils.default.isObject(notationsObject)) {
        throw new _notation.default(ERR.NOTA_OBJ + 'Expected an object.');
      }

      var value;

      _utils.default.each(Object.keys(notationsObject), function (notation) {
    ...
    value: function separate(notations) {
      ...
      if (!_utils.default.isArray(notations)) {
        throw new _notation.default(ERR.NOTA_OBJ + 'Expected an array.');
      }

      var o = new Notation({});

      _utils.default.each(notations, function (notation) {
    ...
    value: function filter(globNotations, options) {
      ...
      var copy = _utils.default.deepCopy(original);

      var re = _utils.default.re; // ensure array, normalize and sort the globs in logical order. this
      // also concats the array first (to prevent mutating the original
      // array).

      var globs = _notation2.default.normalize(globNotations);
      ...
      _utils.default.each(globs, function (globNotation) {
        g = new _notation2.default(globNotation); // set flag that indicates whether the glob ends with `.*`
    ...
    value: function clone() {
      return new Notation(_utils.default.deepCopy(this.value));
    }
    /**
    ...
    value: function copyTo(destination, notation) {
      ...
      if (!_utils.default.isCollection(destination)) throw new _notation.default(ERR.DEST);
      var result = this.inspect(notation);

      if (result.has) {
        var newN = _utils.default.getNewNotation(newNotation, notation);
    ...
    value: function copyFrom(target, notation) {
      var newNotation = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
      var overwrite = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : true;
      if (!_utils.default.isCollection(target)) throw new _notation.default(ERR.DEST);
    ...
    value: function split(notation) {
      if (!Notation.isValid(notation)) {
        throw new _notation.default(ERR.NOTATION + "'".concat(notation, "'"));
      }
    ...
    value: function join(notes) {
      return _utils.default.joinNotes(notes);
    }
    /**
    ...
    value: function eachNote(notation, callback) {
      ...
      _utils.default.each(notes, function (note, index) {
...
Notation.Error = _notation.default;
/**
...
function _each(collection, callback, parentNotation, topSource) {
  ...
  _utils.default.eachItem(collection, function (value, keyOrIndex) {
    var note = typeof keyOrIndex === 'number' ? "[".concat(keyOrIndex, "]") : keyOrIndex;
    var currentNotation = Notation.join([parentNotation, note]);

    var isCollection = _utils.default.isCollection(value); // if it's not a collection we'll execute the callback. if it's a
